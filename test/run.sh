...
if [[ $(echo "${image}" | grep 'jre') == "" ]]; then
    if [[ "$(docker run --rm --user gradle --volume "${PWD}/java-quickstart:/home/gradle/project" --workdir "/home/gradle/project" "${image}" gradle clean test | grep 'BUILD SUCCESSFUL')" == "" ]]; then
